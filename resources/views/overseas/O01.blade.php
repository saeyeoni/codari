...
//해외영업,콜백함수로 정의, direct저장시 브랜드id에 따른 name을 가져올때 비동기식처리를 위해 콜백함수로 정의
...
function store(param){
  ...
  if(param == 'store'){
    ...
    if(validation(vSet)==true){
      ...
      theForm.action= "{{$pgm_info->m_pgm_id}}"+"?pgm=overseas&process=store";
      theForm.submit();
    }
    @endisset
    ...
    vSet.push(["textarea","content","내용","empty"]);
    if(validation(vSet)==true){
      theForm.action= "/overseas/O01"+"?pgm=overseas&process=direct_store";
      theForm.submit();
    }
  }else{
