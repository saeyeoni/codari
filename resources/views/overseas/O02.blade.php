...
<H5 id="brand_nm">　</h5>
            ...
            for(let p=0; p<prd_max; p++){
              ...
              for(let c=0; c<=data.col_max; c++){
                ...
                if(data_idx[idx] != null && data_idx[idx].column_index == c){
                  ...
                  let id = data_idx[0];
                  note = note + '<td class="regist"><a href="javascript:popupWindow(\'O01/create\',\'store\',\''+id.brand_id+'\',\''+id.note_type+'\',\''+id.use_type+'\',\''+id.prd_type+'\',\''+id.map_name+'\')">등록</a></td>';
                }else{
  ...
  function brandDelete(paramId, paramNm){
    ...
    if(answer){
      $.ajax({
        type: "PATCH",
        url : "/overseas/O02/brands/"+paramId+"",
        data: {
          pgm:"overseas",
          _process:"delBrand",
          _token : $('meta[name="csrf-token"]').attr('content'),
